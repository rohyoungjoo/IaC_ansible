...
| 파트 | 담당 | 주요 역할 |
|------|------|-----------|
| Monitoring | 지영 | Prometheus, Grafana 기반 모니터링 구축 |
| FTP & MAIL | 구현 | FTP / Mail 서버 구축 및 서비스 설정 |
| WEB | 영주 | Apache + Tomcat 기반 WEB/WAS 자동화 구축 |
| DB | 소현 | MariaDB 설치 및 DB 환경 구성 |
| NTP | 자윤 | NTP 서버 구축 및 시간 동기화 설정 |
| WEB 이중화 | 승화 | HAProxy 기반 WEB 이중화 및 로드밸런싱 |
| DNS | 영성 | 내부 DNS 서버 구축 및 도메인 관리 |
